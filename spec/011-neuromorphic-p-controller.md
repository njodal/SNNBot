...
Wire the diagonals `|d| ≤ 1` to nothing and the controller has a dead zone, at no cost beyond the wiring. That is nearly the law the neck of [Vehicle 2](006-vehicle-2.md) runs, `neck rate = Kr × how far the eye is outside its comfortable range`, in spikes: `p` is the proprioceptor of the head joint, `r` is wired to the level it rests in, the diagonals inside `HEAD_COMFORT_DEG` reach nothing, and the rest reach the neck's effectors.

Nearly, and [Version C](006-vehicle-2.md) of that vehicle, where this is now built, says what the rest of it is. A diagonal wired to nothing settles *when* the joint moves and not *how much*, so the first diagonal outside the range still wakes the rung for its whole error and the joint lurches at the edge, which is the one thing the subtraction in that law is there to prevent. The subtraction has nowhere to live but the ladder: the rung for `d` becomes `k × (d − dead)`. A dead zone is free; taking the dead zone off the error costs a different ladder. And it leaves the chatter of [Version A](005-vehicle-1.md) where it is, since the diagonals next to zero are exactly where the head trembles.
